...
/// @dev Agents can delegate authority to other addresses with expiration
contract AgentDelegation {
    ...
    /// @notice Creates a delegation from msg.sender to a delegate
    ...
    function delegate(address _delegate, uint256 _expiresAt) external {
        _delegateFor(msg.sender, _delegate, _expiresAt);
    }
    ...
    function delegateBySig(
        ...
        bytes32 structHash = keccak256(
            abi.encode(DELEGATION_TYPEHASH, _agent, _delegate, _expiresAt, nonce)
        );
    ...
    function revokeBySig(address _agent, bytes calldata _signature) external {
        uint256 nonce = nonces[_agent];
        bytes32 structHash = keccak256(
            abi.encode(DELEGATION_TYPEHASH, _agent, address(0), 0, nonce)
        );
        bytes32 digest = EIP712Helper.hashTypedData(DOMAIN_SEPARATOR, structHash);
    ...
    /// @param _delegate The address to delegate authority to
    /// @param _expiresAt The timestamp when the delegation expires
    function _delegateFor(address _agent, address _delegate, uint256 _expiresAt) internal {
        ...
        emit DelegationCreated(_agent, _delegate, _expiresAt, nonces[_agent]);
    }
